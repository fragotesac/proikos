...
if ($action == 'second') {
    ...
    $isPetroperu = false;
    if (strtolower($entitySelect) == 'petroperu') {
        $isPetroperu = true;
    }
    ...
    if ($isPetroperu) {
        ...
        $sedes = [
            ...
            'Oficina Principal (Sede Principal)' => 'Oficina Principal (Sede Principal)',
            'Oleoducto' => 'Oleoducto',
            'Plantas y Terminales' => 'Plantas y Terminales',
            'Conchan' => 'Conchan',
            'Selva' => 'Selva',
            'Talara' => 'Talara'
        ];
        $sedeSelect = $form->addSelect('company_headquearter', $plugin->get_lang('CompanyHeadquarter'), $sedes);
        $form->setRequired($sedeSelect);
        $form->addHtml('</div></div>');
    }
